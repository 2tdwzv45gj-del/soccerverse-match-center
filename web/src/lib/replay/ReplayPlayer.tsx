    ...
    >
      ...
      >
        ...
        >
          ...
          {(data.injuries ?? [])
            .filter(
              (injury) =>
                injury.side === "home" &&
                injury.minute <= state.match_minute
            )
            .map((injury) => (
              <div
                key={"home-injury-" + injury.player_id}
                style={{
                  marginTop: 4,
                  fontSize: 12,
                  fontWeight: 700,
                }}
                title={"Infortunio — " + injury.player_name}
              >
                🩹 {injury.player_name}
              </div>
            ))}
        ...
        >
          ...
          {(data.injuries ?? [])
            ...
            .map((injury) => (
              <div
                key={"away-injury-" + injury.player_id}
                style={{
                  marginTop: 4,
                  fontSize: 12,
                  fontWeight: 700,
      ...
      >
        ...
        >
          ...
          {(data.injuries ?? [])
            .filter((injury) => injury.minute <= state.match_minute)
            .map((injury, index) => {
              const position = Math.min(100, Math.max(0, (injury.minute / 90) * 100));

              return (
                <span
                  key={"injury-" + String(injury.player_id) + "-" + String(index)}
                  title={"Infortunio — " + injury.player_name}
                  style={{
                    position: "absolute",
                    left: `${position}%`,
